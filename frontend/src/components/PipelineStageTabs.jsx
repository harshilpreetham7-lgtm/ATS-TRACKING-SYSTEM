      ...
      <div className="flex flex-wrap gap-2">
        {stages.map((stage) => (
          <button
            ...
            className={`rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] transition ${selectedStage === stage.id ? 'bg-slate-700 text-slate-100' : 'bg-slate-900 text-slate-400 hover:bg-slate-800'}`}>
            {stage.title}
          </button>
        ))}
      </div>
      <div className="rounded-[1.75rem] bg-slate-900/80 p-5 text-slate-300 ring-1 ring-white/10">
        ...
        {selected?.criteria?.length ? (
          <div className="mt-5 rounded-3xl bg-slate-950/70 p-4 ring-1 ring-white/5">
            <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Step-by-step checklist</p>
